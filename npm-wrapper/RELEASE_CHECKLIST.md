...
- [ ] **Promoting a dist-tag (e.g. rc → latest) is a SEPARATE manual step** — the
      `promote_to` workflow_dispatch input exists but currently fails (`npm dist-tag add` has no
      OIDC support yet). Use the manual bootstrap-token flow above instead.
