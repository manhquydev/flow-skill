...
| **Current GA** | **0.5.1** — ships skill product **v0.28.1** |
...
| Pin installer `@0.5.1` if needed | Pin npm `@0.28.1` (skill version ≠ package version) |
...
| `No matching version found` for `@0.28.1` | That is the **skill** version. Use `@latest` or `@0.5.x` |
...
```jsonl
{"event":"plan","version":"0.5.1","skillVersion":"0.28.1","dryRun":false,"scope":"global","targets":["claude"]}
