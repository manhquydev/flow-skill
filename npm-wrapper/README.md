...
```
# Newest GA — current: v0.4.0, ships skill v0.27.0
...
>
> **Two version axes:** package `version` in this folder is the **installer** (e.g. `0.4.0`).
> Skill product version lives in `skills/flow/SKILL.md` (`metadata.version`, e.g. `0.27.0`).
...
- **`No matching version found` on `@0.26.0`**: that is the **skill product** version, not the npm package version. Install the installer by its own version (`@latest` / `@0.2.x`) — skill content version is printed by `--help` as `ships skill v…`.
...
```jsonl
{"event":"plan","version":"0.4.0","skillVersion":"0.27.0","dryRun":false,"scope":"global","targets":["claude","codex"]}
