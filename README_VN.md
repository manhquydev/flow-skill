...
> Trạng thái: **v0.8.0** — engine + **vòng tri thức** bền vững khép kín (recall · audit/propose · KB
...
├── tests/run_all.sh             # 19 bộ test / 367 check
...
> trên Windows/Git Bash; bộ test dev là 19 bộ / 367 check (`bash tests/run_all.sh`).
...
## Chạy test
```bash
bash tests/run_all.sh    # 19 bộ test / 367 check; cần bash (+ python cho bộ harness/propose)
```
