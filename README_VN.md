...
[![CI](https://github.com/manhquydev/mq_flow/actions/workflows/ci.yml/badge.svg)](https://github.com/manhquydev/mq_flow/actions/workflows/ci.yml) — 25 bộ test / 605 check trên macOS · Ubuntu · Windows
...
> Trạng thái: **v0.16.1** — **vòng đời card dễ đọc**: hai verb `/flow card start|done` — `start` đánh dấu card
...
> **25 bộ test / 605 check xanh** trên macOS · Ubuntu · Windows. MIT.
...
├── tests/run_all.sh             # 25 bộ test / 605 check
...
> trên Windows/Git Bash; bộ test dev là 25 bộ / 605 check (`bash tests/run_all.sh`).
...
## Chạy test
```bash
bash tests/run_all.sh    # 25 bộ test / 605 check; cần bash (+ python cho bộ harness/propose)
```
