...
[![CI](https://github.com/manhquydev/mq_flow/actions/workflows/ci.yml/badge.svg)](https://github.com/manhquydev/mq_flow/actions/workflows/ci.yml) — 14 bộ test / 224 check trên macOS · Ubuntu · Windows
...
harness bền vững** (intake/story/trace/decision/backlog), điều phối agent (ck: + bmad), và nhận biết
loại dự án.

> Trạng thái: **v0.3** — engine + **vòng tri thức** bền vững khép kín (recall · audit/propose · KB
> liên-dự-án) + capture tự động tại gate + drift checks (contract/tokens/coherence) + chế độ
> brownfield `assess` + concurrency lock + tích hợp agent + DESIGN law + nhận biết loại dự án.
> **14 bộ test / 224 check xanh.** MIT.
...
├── tests/run_all.sh             # 14 bộ test / 224 check
...
/flow contract|tokens|coherence   drift checks (path-resolution · design token · version)
/flow doctor           kiểm môi trường trên macOS/Linux/Windows
```

## Lệnh (đầy đủ)

"Bắt đầu nhanh" ở trên là đường đi thường gặp; đây là tham chiếu đầy đủ — cả 21 lệnh engine dispatch (`bash skills/flow/runner/flow.sh <command>`):
...
> trên Windows/Git Bash; bộ test dev là 14 bộ / 224 check (`bash tests/run_all.sh`).
...
## Chạy test
```bash
bash tests/run_all.sh    # 14 bộ test / 224 check; cần bash (+ python cho bộ harness/propose)
```
