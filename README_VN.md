...
**28 bộ test / 680 check, xanh cục bộ trên macOS · Ubuntu · Windows (Git Bash), và trên CI hosted (GitHub Actions, xanh trên `master`).**
...
> Trạng thái: **v0.18.0** — **tích hợp loop-engineering `ck-loop`**: đoạn đuôi "Implement→Test→Audit→Fix"
...
> **28 bộ test / 680 check xanh cục bộ** (macOS · Ubuntu · Windows qua Git Bash) và trên CI hosted GitHub Actions. MIT.
...
├── tests/run_all.sh             # 28 bộ test / 680 check
...
"Bắt đầu nhanh" ở trên là đường đi thường gặp; đây là tham chiếu đầy đủ — cả 27 lệnh engine dispatch (`bash skills/flow/runner/flow.sh <command>`):
...
> trên Windows/Git Bash; bộ test dev là 28 bộ (`bash tests/run_all.sh`).
...
## Chạy test
```bash
bash tests/run_all.sh    # 28 bộ test / 680 check; cần bash (+ python cho bộ harness/propose)
```
